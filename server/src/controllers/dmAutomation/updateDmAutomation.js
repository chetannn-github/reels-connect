...
export const updateDmAutomation = async (req, res) => {
  try {
    ...
    const { keyword, dmMessages, isActive, dmAutomationId} = req.body;
    ...
    if (keyword !== undefined) {
      const exists = await DMAutomation.findOne({ user: userId, keyword, _id: { $ne: dmAutomationId } });
      if (exists) {
    ...
    return res.json({ message: "Rule updated successfully", rule });
  } catch (error) {
